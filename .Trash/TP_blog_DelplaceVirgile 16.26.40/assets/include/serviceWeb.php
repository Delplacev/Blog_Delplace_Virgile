...
class actionSite {
    ...
    private function verifIpVote($idArticle, $ipVote) { 
        $query = "SELECT * FROM vote WHERE idArticle = $idArticle AND ipVote = $ipVote";
        $result = mysql_query($query);
    ...
    private function getNbVote($idArticle) {
        if ($_GET["idArticle"] != "") {
            $idArticle = $_GET["idArticle"];
            $query = "SELECT count(*) as nbVote FROM `vote` WHERE idArticle = $idArticle";
            $result = mysql_query($query);
    ...
    private function addVote($idArticle) {
        ...
        if ($_POST[idArticle] != "") {
            ...
            if (!$this->verifIpVote($idArticle, $ipVote)) {
                mysql_real_escape_string();

                // Ajout de l'article
                $query = "INSERT INTO `vote`(`idVote`, `idArticle`, `ipVote` ) VALUES ('','" . $idArticle . "','" . $ipVote . "');";
